...
y_numpy = np.sin(X_numpy) + 0.8 * np.random.randn(100, 1)
...
optimizer = torch.optim.SGD([a, b], lr=0.01) # 优化器，基于 a b 梯度 自动更新
...
plt.scatter(X_numpy, y_numpy, alpha=0.3, s=10, label='Training data', color='blue')
X_plot = np.linspace(0, 10, 500).reshape(-1, 1)
y_plot = a_learned * np.sin(X_plot) + b_learned
plt.plot(X_plot, y_plot, 'r-', linewidth=2, label=f'Model: y = {a_learned:.3f}·sin(x) + {b_learned:.3f}')
plt.xlabel('x', fontsize=12)
plt.ylabel('y', fontsize=12)
plt.title('Sine Function Fitting Result', fontsize=14, fontweight='bold')
