...
# 加载和预处理数据
dataset_df = pd.read_csv("./text_classify_dataset.csv", header=None)
#
...
model = BertForSequenceClassification.from_pretrained('../../../models/google-bert/bert-base-chinese', num_labels=4)
...
if best_model_path:
    ...
    torch.save(best_model.state_dict(), './bert.pt')
...
request_text = "跟进合作的项目进度，确保按时交付"
test_encoding = tokenizer([request_text], truncation=True, padding=True, max_length=30)
# test_dataset = NewsDataset(test_encoding, [0])
test_dataset = Dataset.from_dict({
    'input_ids': test_encoding['input_ids'],           # 文本的token ID
    'attention_mask': test_encoding['attention_mask'], # 注意力掩码
    'labels': [0]                               # 对应的标签
})
...
for batch in test_dataloader:
    ...
    logits = logits.detach().cpu().numpy()  # 转numpy方便计算
    pred += list(np.argmax(logits, axis=1).flatten())
# 打印预测结果
print(f"输入文本：{request_text}")
print(f"预测类别索引：{pred[0]}")
label_map = {0:'积极',1:'消极',2:'工作',3:'生活'}
print(f"预测类别名称：{label_map[pred[0]]}")